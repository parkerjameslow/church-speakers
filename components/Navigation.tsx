    ...
    <>
      ...
      <aside className="hidden md:flex flex-col w-56 min-h-screen bg-gray-900 text-white fixed left-0 top-0">
        <div className="px-4 py-5 border-b border-gray-800">
          ...
          <div className="text-xs text-gray-500">Speaker Tracker</div>
        </div>
      ...
      {/* Mobile bottom bar */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-gray-900 flex border-t border-gray-800 z-50">
        {navItems.map((item) => {
          const active = pathname.startsWith(item.href)
          return (
            <Link
