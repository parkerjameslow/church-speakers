                ...
                <ol className="space-y-3">
                  {meeting.assignments.map((a: any, i: number) => (
                    <li key={a.id} className="flex items-center gap-3">
                      ...
                      <div>
                        <Link href={`/members/${a.member_id}`} className="font-medium text-gray-900 hover:text-blue-700">
                          {a.member_name}
                        </Link>
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              <div>
                ...
                <input type="date" className="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={date} onChange={(e) => setDate(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                <textarea rows={2} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={notes} onChange={(e) => setNotes(e.target.value)} />
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                {assignments.map((a, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2">
                    ...
                    <div className="flex-1">
                      ...
                      <input type="text" placeholder="Topic…" className="w-full text-xs border border-gray-200 rounded px-2 py-1 mt-1 focus:outline-none focus:ring-1 focus:ring-blue-400" value={a.topic} onChange={(e) => updateTopic(idx, e.target.value)} />
                    </div>
              ...
              <div className="relative">
                <input type="search" placeholder="Add a speaker…" className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" value={search} onChange={(e) => setSearch(e.target.value)} />
                {filteredMembers.length > 0 && (
                  <div className="absolute top-full left-0 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-10 max-h-40 overflow-y-auto mt-1">
                    {filteredMembers.slice(0, 8).map((m) => (
                      <button key={m.id} type="button" onClick={() => addSpeaker(m)} className="w-full text-left px-3 py-2 text-sm hover:bg-blue-50">{m.name}</button>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleSave} disabled={saving} className="flex-1 bg-blue-700 hover:bg-blue-800 text-white font-semibold py-2 rounded-xl text-sm transition disabled:opacity-50">{saving ? 'Saving…' : 'Save'}</button>
            </div>
          </div>
        )}
      </main>
